...
//GET DATI DOCENTI
router.get('/votazioni/docenti/classico', (req, res) => {
  ...
  const queryToSend = [
    (cb) => {
      ...
      pool.query(`SELECT idDocente , voto , idDomanda ,COUNT(*) as countValue FROM votazioni INNER JOIN studenti ON votazioni.idStudente = studenti.id WHERE studenti.idClasse LIKE '__C%' ${whereString} GROUP BY voto , idDomanda , idDocente ORDER BY idDocente , idDomanda , voto ASC`, (err, rows, fields) => {
